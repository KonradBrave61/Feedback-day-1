...
BACKEND_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://chat-display-fix-1.preview.emergentagent.com')
...
class BackendTester:
    ...
    def register_and_login_users(self):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self.log_result("User B Registration", True, f"User ID: {self.user_b_id}", response.status_code)
            else:
    ...
    def test_chat_start(self):
        ...
        try:
            headers = {"Authorization": f"Bearer {self.user_a_token}"}
            params = {"partner_id": self.user_b_id}
            
            response = self.session.post(f"{API_BASE}/chat/start", params=params, headers=headers)
    ...
    def test_unblock_user(self):
        ...
        try:
            ...
            response = self.session.post(f"{API_BASE}/chat/unblock", params=params, headers=headers)
