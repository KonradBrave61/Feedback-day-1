...
const TeamPreviewModal = ({ isOpen, onClose, team, onPrivacyToggle }) => {
  ...
  const renderBench = () => {
    ...
    return (
      <div className="space-y-2">
        <h4 className="text-sm font-medium text-white">Bench</h4>
          ...
          {[...Array(5)].map((_, index) => {
            ...
            return (
              <div key={index} className="text-center">
                <div 
                  className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold border border-white/30 mb-1"
                  style={{ backgroundColor: player ? getPositionStyle(player.position).backgroundColor : logoColors.blackAlpha(0.5) }}
                >
  ...
  const renderPlayerDetails = () => {
    const allPlayers = [...(teamDetails?.players || []), ...(teamDetails?.bench || [])];
            ...
            <div key={index} className="p-3 rounded border" style={{ 
              ...
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <p className="text-xs" style={{ color: logoColors.lightBlue }}>
                    {player.position} • Level {player.userLevel || player.level || 1}
                  </p>
                </div>
              ...
              {player.userEquipment && Object.keys(player.userEquipment).length > 0 && (
                <div className="mb-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {Object.values(player.userEquipment).map((equipment, eqIndex) => (
                      equipment && (
              ...
              {player.userHissatsu && player.userHissatsu.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {player.userHissatsu.map((technique, techIndex) => (
