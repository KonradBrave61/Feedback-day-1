...
async def follow_user(
    ...
    target_user = await db.users.find_one({"id": follow_request.user_id})
    if not target_user:
        raise HTTPException(
    ...
    if follow_request.user_id in current_user.following:
        # Unfollow
        await db.users.update_one(
            {"id": current_user.id},
            {"$pull": {"following": follow_request.user_id}}
        )
        await db.users.update_one(
            {"id": follow_request.user_id},
        ...
        await db.users.update_one(
            {"id": current_user.id},
            {"$addToSet": {"following": follow_request.user_id}}
        )
        await db.users.update_one(
            {"id": follow_request.user_id},
